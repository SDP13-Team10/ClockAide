// 5/9/13  May need to have python code set to /dev/ttyACMx or /dev/USBx 
    ...
    switch(mode){
       ...
       case CHECK_ID:            // Check ID 
             ...
             if (response == GOOD){
                 ...
                 else if (m == '9'){                  // For database extraction
                     sendToPi(TEACHER);
                     mode = getFromPi();
                 }
                 else{
       ...
       case TEACHER: 
       {
         char e;
             ...
             delay(2000);
            myLCD.clearLCD();
            myLCD.writeToScreen("Enter Command", 0,0);
            myLCD.selectLineTwo();
            Serial.print(Control());
             ...
             if(e == 1){
                 sendToPi(DATA);
     //            sendToPi(6);                Set to test access to Teacher mode, and exiting python script. EXIT was set to COMMAND 6 in python script    
                 myLCD.clearLCD();
                 myLCD.writeToScreen("Processing...", 0,0);
                 ...
                 if (response == GOOD){            
                     myLCD.clearLCD();
                     myLCD.writeToScreen("Saving data...", 0,0);
        
                 }
                 
                 else if (response == WRONG){
                 myLCD.clearLCD();
                 myLCD.writeToScreen("Extraction Failed", 0,0);
                 myLCD.selectLineTwo();
                 myLCD.writeToScreen("Try Again", 1,0);
                 ...
                 mode = TEACHER;               
                  ...
                  myLCD.clearLCD();
                  myLCD.writeToScreen("Returning to normal mode...", 0,0);
                  mode = NORMAL;
                }
...
int getOption(){
   ...
   int j = 0;
   myLCD.clearLCD();
                myLCD.writeToScreen("(1) Yes (2) No", 0,0);
                myLCD.selectLineTwo();
   while(i<0){
       keyStroke = keypad.getKey();  
            switch(keyStroke){
...
String getTime(){
 ...
 return (String(a) + String(b)); // return time string 
...
String Control(){
  ...
  int pos = 0;
  //myLCD.selectLineTwo();
  //myLCD.writeToScreen(String(a) + String(b), 0,0);
  while (pos < 2){      // User must fill all spaces 
      switch(pos){
         case 0:
              a = keypad.waitForKey();
              if(a == '#' || b == '*'){
                ...
                myLCD.writeToScreen(String(a) + String(b), 1,0);
                pos = 0;
              }
              else{
                myLCD.writeToScreen(String(a) + String(b), 1,0);          
 ...
 return (String(a) + String(b)); // return user response
